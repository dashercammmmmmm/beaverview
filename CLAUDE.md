...
It dry-runs the committed sample and dry-runs the real ignored CSV if present, without replacing `device_ips`. It also verifies that sample, real, duplicate-mapping, and unreviewed-public-IP validation output does not print raw IP values.
...
`scripts/check_readiness_actions.py` validates those pending-action references, verifies literal and supported loop-generated `pending(...)` messages have mapped actions, and is part of the readiness gate.
Local validator failures include bounded, redacted stdout/stderr tails. `scripts/sanitize_output.py` is the shared redaction helper for readiness/report tooling, and `scripts/check_sanitize_output.py` plus `scripts/check_readiness_diagnostics.py` verify secret-like values, authorization headers, and non-local IPv4 addresses stay redacted.
...
The first live-room validation runbook is `docs/examples/first-live-room-validation.md`. It defines the non-critical-room checklist, connector order, evidence rules, rollback path, and no-secrets/no-raw-IP requirements. `scripts/check_first_live_room_preflight.py --list-candidates` lists sanitized candidate room IDs and connector hints without raw IPs; add `--connector <name>` to filter the shortlist to one first connector, and `--hardware-csv api/hardware_ips.csv` to preview device-backed matches from a validated secure CSV before import. The CSV preview fails on blank required fields, unknown room IDs, duplicate room/device mappings, invalid IP rows, and unreviewed public IP rows without printing raw IPs, matching the import validator. `scripts/check_first_live_room_preflight.py` validates the selected `FIRST_LIVE_ROOM_ID` / `FIRST_LIVE_CONNECTOR` target when those ignored `.env` values are set. `scripts/render_first_live_room_report.py --readiness-json /tmp/beaverview-readiness.json --candidates-json /tmp/beaverview-candidates.json` renders a private no-secrets validation report template around the candidate shortlist, selected-room preflight result, saved readiness snapshot, pending next actions, and a conservative go/no-go decision that requires the selected room/connector to match the attached candidate snapshot. Connector aliases such as `crestron` are normalized by `scripts/first_live_connectors.py` and validated by `scripts/check_first_live_connectors.py`. `scripts/check_first_live_room_preflight_cases.py` and `scripts/check_first_live_room_report.py` exercise pass/pending/fail, candidate-list, and report no-leak behavior against isolated temp DBs and are part of readiness. `python3 scripts/check_live_validation_doc.py` validates that the runbook still covers those gates and is part of `scripts/check_pilot_readiness.py`.
