...
if [ -n "${BEAVERVIEW_BROWSER_PORT:-}" ]; then
  PORT="$BEAVERVIEW_BROWSER_PORT"
else
...
cd "$API_DIR"
"$API_DIR/venv/bin/uvicorn" main:app --host 127.0.0.1 --port "$PORT" > "$TMP_DIR/beaverview-browser-smoke.log" 2>&1 &
SERVER_PID=$!
trap 'kill "$SERVER_PID" >/dev/null 2>&1 || true; rm -rf "$TMP_DIR"' EXIT

for _ in $(seq 1 40); do
  if curl -fsS "$BASE_URL/api/health" >"$TMP_DIR/beaverview-browser-health.json" 2>/dev/null; then
    break
  fi
  sleep 0.25
done
