...
- `scripts/render_first_live_room_report.py --readiness-json /tmp/beaverview-readiness.json` renders a private no-secrets Markdown report template around the selected first live-room preflight result and a saved readiness snapshot, including pending next actions and a conservative go/no-go decision.
