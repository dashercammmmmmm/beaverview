...
**Last updated:** 2026-06-24 after adding Hardware IP CSV candidate preview.
...
**Markdown pilot readiness report:**
```
cd "/Users/benjaminfranklinautomation/projects/beaverview" && python3 scripts/check_pilot_readiness.py --markdown
```
...
- `scripts/render_first_live_room_report.py --readiness-json /tmp/beaverview-readiness.json --candidates-json /tmp/beaverview-candidates.json` renders a private no-secrets Markdown report template around the candidate shortlist, selected first-live preflight result, saved readiness snapshot, pending next actions, and a conservative go/no-go decision that requires the selected room/connector to match the attached candidate snapshot; connector aliases such as `crestron` normalize before matching.
...
- `api/hardware_ip_csv.py` owns shared Hardware IP CSV row parsing and validation for both import and first-live preview.
- `scripts/check_hardware_ip_csv.py` directly validates the shared parser and is part of pilot readiness.
...
- `scripts/check_first_live_room_preflight.py --list-candidates` lists sanitized candidate rooms and connector hints before OSU selects the target; add `--connector <name>` for a connector-specific shortlist and `--hardware-csv api/hardware_ips.csv` for a no-raw-IP preview from the validated secure CSV. The CSV preview rejects blank required fields, unknown room IDs, duplicate room/device mappings, invalid IP rows, and unreviewed public IP rows so it stays aligned with import validation.
